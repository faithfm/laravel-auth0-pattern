...
namespace FaithFM\Auth0Pattern\Guards;
...
use Auth0\Laravel\Guards\{GuardAbstract, AuthenticationGuardContract};
...
/**
 * Class PatchedAuthenticationGuard
 *
 * This class is a temporary bug-fix to overcome a current (v7.12) bug where the Auth0 SDK does not correctly handle the 'accessToken' vs 'idToken' 
 * when AUTH0_AUDIENCE is blank (ie: using the default https://faithfm.au.auth0.com/userinfo endpoint instead of an API endpoint).
 *    TODO - create an issue like we did for the other one!!!!!!!     See: https://github.com/auth0/laravel-auth0/issues/384
 * 
 * The class is registered as the 'auth0.authenticator.patched' authentication guard driver (in Auth0PatternServiceProvider)
 *
 * This file is an identical clone of Auth0\Laravel\Guards\AuthenticationGuard class, with the following changes to the 'pullState()' method:
 *
 *    // *** FFM PATCH START ***
 *    // Use the access token if it is a JWT, otherwise use the ID token (which is always a JWT)
 *    $accessTokenIsJwt = (3 == count(explode('.', $credentials->accessToken)));
 *    $jwt = $accessTokenIsJwt ? $credentials->accessToken : $credentials->idToken;
 *    $decoded = (new Parser(new SdkConfiguration(strategy: SdkConfiguration::STRATEGY_NONE), $jwt))->export();
 *    // *** FFM PATCH STOP ***
 * 
 * Note: we had to clone the file, because Auth0 decided to make it a "final" class, which prevents us from extending it.
 * 
 * For more details, refer to the following github issue we raised:  https://github.com/auth0/laravel-auth0/issues/454
 * 
 */
 ...
 * @api
 */
final class PatchedAuthenticationGuard extends GuardAbstract implements AuthenticationGuardContract
{
